...
var App = function (_Component) {
  ...
  function App(props) {
    ...
    _this.deselect = function () {
      ...
      var index = (0, _lodash.indexOf)(nav, itemSelectedId);
    ...
    _this.onClickItem = function (item, itemIndex) {
      ...
      var navIndex = (0, _lodash.indexOf)(nav, item.parent);
      ...
      if (item.parent === last) {
        ...
        // Click before last
        if (item.children.length > 0) {
          _this.setState({ nav: [].concat(_toConsumableArray(nav.slice(0, navIndex + 1)), [item.id]) });
        } else {
          _this.setState({ nav: nav.slice(0, navIndex + 1) });
        }
      }

      _this.setState({ itemSelectedIndex: itemIndex, itemSelectedId: item.id });
    };
